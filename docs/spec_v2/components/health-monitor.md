...
>
> ⚠️ **HealthMonitor가 멈추면 DB는 "고정된 거짓말"이 된다**
...
```mermaid
flowchart TB
    HM["HealthMonitor<br/>(30초마다 관측)"]

    subgraph Resources["Actual Resources"]
        R1["Container: exists? running?"]
        R2["Volume: exists?"]
        R3["error_info: is_terminal?"]
        R4["archive_key: exists?"]
    end

    subgraph Compute["observed_status 결정"]
        C1["error_info.is_terminal = true → ERROR"]
        C2["container + !volume → ERROR (불변식 위반)"]
        C3["container + running → RUNNING"]
        C4["volume (no container) → STANDBY"]
        C5["!volume + archive_key → PENDING"]
        C6["!volume + !archive_key → ERROR (DataLost)"]
    end

    DB["Database<br/>UPDATE workspaces SET observed_status = ?, observed_at = NOW()"]

    HM -->|"observe"| Resources
