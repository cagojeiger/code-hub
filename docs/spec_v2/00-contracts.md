...
| 2 | StateReconciler | `op_id` 유지 (GC 보호) |
| 3 | HealthMonitor | `observed_status=ERROR` 관측/기록 |

> **Single Writer 준수**: SR이 operation/op_id/error_info 설정, HM이 observed_status 설정
...
| 상태 | 설명 |
|------|------|
| ERROR | 어느 상태에서든 전이 가능 (불변식 위반, timeout 등) |
| DELETED | 삭제 완료 상태 |
...
| `observed_status = ERROR` | ERROR 탈출 삭제 |
...
| 위반 감지 | HealthMonitor (ContainerWithoutVolume → ERROR) |
...
| 조건 | 동작 |
|------|------|
| ERROR 상태 | archive 삭제 금지 (복구 가능성 유지) |
