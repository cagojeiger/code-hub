# Roadmap 001: MVP

## Status: In Progress

> spec.md MVP 완료 기준 달성을 목표로 합니다.

---

## Milestones

### M0: Technical Spike
**Status**: Pending

**Tasks**:
- [ ] 단순 HTTP 리버스 프록시 (Exit: FastAPI → httpx → 외부 서버 프록시 동작)
- [ ] WebSocket 프록시 (Exit: 양방향 메시지 전달 동작)
- [ ] code-server 연동 테스트 (Exit: Docker code-server에 프록시 연결, 터미널/에디터 동작)
- [ ] 학습 내용 정리 (Exit: 성공/실패 케이스, 코드 스니펫, 주의사항 문서화)

**Notes**:
- 목적: M5 Proxy의 기술적 위험 사전 검증
- 최소한의 코드로 핵심 기술 동작 확인 (throwaway code)
- 성공 시 M5 구현 방향 확정, 실패 시 대안 검토 (nginx 등)
- code-server도 Docker 컨테이너로 테스트 (로컬 설치와 동작 차이 방지)
- 실행 환경: 로컬 Python + Docker로 code-server 컨테이너 실행 (M1 docker-compose 없이 독립 실행)

---

### M1: Foundation
**Status**: Pending

**Tasks**:
- [ ] Dockerfile + docker-compose.yml (Exit: `docker compose up` 성공, hot reload 동작, Docker socket mount)
- [ ] pyproject.toml + uv 설정 (Exit: 컨테이너 내 `uv sync` 성공)
- [ ] Config 모듈 구현 (Exit: env-only로도 부팅 가능, 잘못된 값은 명확한 에러)
- [ ] Errors 모듈 구현 (Exit: INVALID_STATE, WORKSPACE_NOT_FOUND 등 spec 에러 코드 정의)
- [ ] DB 설정 + 모델 구현 (Exit: SQLite WAL 모드, 전체 테이블 생성, 테스트 유저 seed)

**Notes**:
- 명시적 네트워크 이름 `codehub-net` 사용 (M3에서 생성하는 워크스페이스 컨테이너도 같은 네트워크로 관리)
- uid/gid 설정 필요 (code-server 호환: 1000:1000)
- data 디렉토리는 bind mount 사용 (Docker volume 아님), .gitignore에 추가

---

### M2: Storage Provider
**Status**: Pending

**Tasks**:
- [ ] Storage Provider 인터페이스 정의 (Exit: Provision/Deprovision/Purge/GetStatus 시그니처)
- [ ] local-dir 백엔드 구현 (Exit: Provision 멱등성 테스트 통과)

**Notes**:
- 디렉토리 생성 시 uid/gid 설정 (M1에서 정의한 1000:1000 사용)
- ⚠️ 경로 불일치 주의: Control Plane 컨테이너 경로 vs 호스트 경로 (Docker bind mount에는 호스트 경로 필요)
- Config에서 HOME_STORE_HOST_PATH 환경변수로 호스트 경로 관리

---

### M3: Instance Controller
**Status**: Pending

**Tasks**:
- [ ] Instance Controller 인터페이스 정의 (Exit: Start/Stop/Delete/ResolveUpstream/GetStatus 시그니처)
- [ ] local-docker 백엔드 구현 (Exit: 컨테이너 생성/시작/정지/삭제 멱등성 테스트 통과)
- [ ] 초기화 스크립트 (Exit: 개발 중 남은 워크스페이스 컨테이너 정리 가능)

**Notes**:
- docker-py 사용 (Docker socket 통해 제어)
- 컨테이너 이름 prefix `codehub-ws-`로 다른 컨테이너와 분리
- 테스트 시 간단한 이미지 사용 (busybox/alpine), code-server는 M5에서
- 시작 시 `codehub-net` 네트워크 존재 확인 + 자동 생성 로직 필요

---

### M4: Workspace API
**Status**: Pending

**Tasks**:
- [ ] Workspace CRUD API (Exit: 생성/조회/수정/삭제 동작)
- [ ] StartWorkspace API (Exit: CREATED/STOPPED/ERROR → PROVISIONING → RUNNING)
- [ ] StopWorkspace API (Exit: RUNNING/ERROR → STOPPING → STOPPED)
- [ ] DeleteWorkspace API (Exit: CREATED/STOPPED/ERROR → DELETING → DELETED)
- [ ] Startup Recovery (Exit: 서버 재시작 시 전이 상태 자동 복구)
- [ ] 더미 UI (Exit: Start/Stop/Delete 버튼 + 실시간 상태 + 에러 표시)

**Notes**:
- Auth 없이 구현, 테스트 유저로 고정 (M6에서 인증/인가 추가)
- ⚠️ CAS 패턴 필수: `WHERE status IN (...)` 으로 동시 요청 시 하나만 성공
- 상태 전이 매트릭스 전체 테스트 케이스 작성 (spec.md 참조)
- 백그라운드 작업: FastAPI BackgroundTasks 사용 (단일 프로세스 내)
- 더미 UI 실시간 상태: polling 방식 (5초 주기)

---

### M5: Proxy
**Status**: Pending

**Tasks**:
- [ ] Workspace Proxy 구현 (Exit: /w/{workspace_id}/ → code-server 연결)
- [ ] Trailing Slash Redirect (Exit: /w/{id} → 308 → /w/{id}/)
- [ ] WebSocket 프록시 (Exit: 터미널/에디터 정상 동작)

**Notes**:
- Auth 없이 구현 (M6에서 인가 추가)
- ⚠️ M0 학습 내용 반드시 참조 (spike 문서 또는 코드 스니펫)
- Health check: HTTP 200 확인 (code-server / 경로)
- 이미지: codercom/code-server (버전은 M5 시작 시 최신 stable 선택)

---

### M6: Auth + E2E
**Status**: Pending

**Tasks**:
- [ ] Session 모델 + 로직 (Exit: 생성/만료/폐기 동작)
- [ ] Login/Logout API (Exit: 세션 쿠키 발급/폐기)
- [ ] Session 조회 API (Exit: GET /api/v1/session → 현재 유저 정보)
- [ ] Auth Middleware (Exit: 쿠키 없으면 401, 만료면 401)
- [ ] Workspace owner 필터링 (Exit: CRUD + Proxy에서 owner 검증)
- [ ] E2E 테스트 (Exit: MVP 완료 기준 4가지 모두 통과)

**MVP 완료 기준** (spec.md):
- [ ] 내 계정으로 생성 → `/w/{workspace_id}/` 접속 성공
- [ ] 다른 계정으로 `/w/{workspace_id}/` 접근 → 403
- [ ] STOP 후 START → Home 유지
- [ ] WebSocket 포함 정상 동작 (터미널/에디터)

**Notes**:
- WebSocket 인증: handshake 시점에 세션 쿠키 검증
- E2E 환경: docker-compose로 전체 스택 실행

---

## References

- [spec.md](../spec.md) - 상세 스펙
- [architecture.md](../architecture.md) - 시스템 아키텍처
- [ADR-001: MVP Tech Stack](../adr/001-mvp-tech-stack.md)
- [ADR-002: MVP Project Structure](../adr/002-mvp-project-structure.md)
