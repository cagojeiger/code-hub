...
**유지 (이름 변경 안 함)**:
- `op_id`, `op_started_at`: archive S3 경로 생성, GC 보호용
- `archive_key`: restore 시 Agent에게 전달 (op_id와 동일한 값)
...
s3_path        = f"{CLUSTER_ID}/{workspace_id}/{op_id}/home.tar.zst"
```
...
```
POST /jobs/archive
요청: { "workspace_id": "ws-001", "op_id": "op-abc123" }
Agent:
...
요청: { "workspace_id": "ws-001", "op_id": "op-abc123" }
Agent:
  1. s3_path = f"{cluster_id}/ws-001/op-abc123/home.tar.zst"
...
요청: {
  "protected": [
    { "workspace_id": "ws-001", "op_id": "op-abc" },
    { "workspace_id": "ws-002", "op_id": "op-def" }
  ]
}
Agent:
  1. protected_paths 계산:
     - f"{cluster_id}/ws-001/op-abc/home.tar.zst"
     - f"{cluster_id}/ws-002/op-def/home.tar.zst"
