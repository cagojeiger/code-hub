...
class JobRunner:
    ...
    async def run_archive(self, workspace_id: str, op_id: str) -> JobResult:
        ...
        try:
            ...
            logger.info("Archive job completed: %s, exit_code=%d", helper_name, exit_code)
            return JobResult(exit_code=exit_code, logs=logs.decode("utf-8", errors="replace"))

        except asyncio.CancelledError:
            logger.warning("Archive job cancelled: %s", helper_name)
    ...
    async def run_restore(self, workspace_id: str, op_id: str) -> JobResult:
        ...
        try:
            ...
            logger.info("Restore job completed: %s, exit_code=%d", helper_name, exit_code)
            return JobResult(exit_code=exit_code, logs=logs.decode("utf-8", errors="replace"))

        except asyncio.CancelledError:
            logger.warning("Restore job cancelled: %s", helper_name)
            await self._force_cleanup(helper_name)
            raise
