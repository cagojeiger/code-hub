...
from codehub.app.metrics.collector import CIRCUIT_BREAKER_ERRORS_TOTAL
...
async def with_retry(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            CIRCUIT_BREAKER_ERRORS_TOTAL.labels(error_type="circuit_open").inc()
            raise
        except Exception as exc:
            last_exc = exc
            error_class = classify_error(exc)
            CIRCUIT_BREAKER_ERRORS_TOTAL.labels(error_type=error_class).inc()
