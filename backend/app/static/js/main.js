...
function renderPagination(pagination) {
  ...
  html += `
    <button data-action="page-prev" ${!pagination.has_prev ? 'disabled' : ''}
  ...
  html += `
    <button data-action="page-next" ${!pagination.has_next ? 'disabled' : ''}
            class="px-3 py-1 bg-vscode-sidebar border border-vscode-border rounded text-vscode-text hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">
      Next
    </button>
  `;
...
async function init() {
  ...
  document.getElementById('detail-name').addEventListener('dblclick', (e) => {
    enableInlineEdit(e.currentTarget, 'name', () => loadWorkspaces(state.currentPage));
  });
  document.getElementById('detail-description').addEventListener('dblclick', (e) => {
    enableInlineEdit(e.currentTarget, 'description', () => loadWorkspaces(state.currentPage));
  });
